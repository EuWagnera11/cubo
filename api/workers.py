...
def generate_image_task(
    ...
    *,
    ...
    """
    Pipeline completo:
      1. nano-banana-pro (4K editorial)
    ...
    try:
        ...
        for i in range(num_variations):
            task_id = _run(fp.nano_banana_pro(
                prompt=full_prompt,
...
def generate_video_task(
    ...
    *,
    ...
    try:
        ...
        engines_chain = [engine, fallback_to, "hailuo", "kling_v2_6"]
        last_err = None
        for eng in engines_chain:
            try:
                fn = getattr(fp, eng)
                task_id = _run(fn(image_url, prompt=prompt, duration=duration))
